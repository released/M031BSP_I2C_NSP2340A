...
#define NSP_I2C_ADDR			(0x5A)		// customize in NSP playlist editor tool , when interface set as I2C
...
{
	// ?O???x?I2C?Y??, u8ReadBuffer ?????FIFO??????????,u16CMD_RX_BYTE ?????????????
	i2c_reg_read(NSP_I2C_ADDR,0x00, (unsigned char *) &u8ReadBuffer[0] , u16CMD_RX_BYTE);
	return READ_SIZE;
}
...
{
	// ?ам????I2C????,u8WriteBuffer ?????????????????FIFO????,u16CMD_TX_BYTE ??????????
	i2c_reg_write(NSP_I2C_ADDR,0x00, (unsigned char *) &u8WriteBuffer[0], u16CMD_TX_BYTE);
	return WRITE_SIZE;
}
//----------------------------------
...
{
	// Write(Send) Process.
	if( N_I2C_WRITE(u16CMD_TX_BYTE) == 0 )
		return 0;
	HOST_I2C_WRITE_DURATION();
	// Read(Receive) Process.
	if( N_I2C_READ(u16CMD_RX_BYTE) == 0 )
		return 0;
...
//	N_I2C_WRITE(u16CMD_TX_BYTE);
//	HOST_I2C_WRITE_DURATION();
//	N_I2C_READ(u16CMD_RX_BYTE);
//	HOST_I2C_READ_DURATION();
//	RTN = CHECK_I2C_READ_RIGHT();
